...
describe("Core.Packs (ADR-0011 built-in class-pack registry)", function()
  ...
  describe("RegisterBuiltinPack(class, thunk)", function()
    ...
    it("rejects a nil thunk the same way, with type 'nil' in the message", function()
      local ok = Packs.RegisterBuiltinPack("WARRIOR", nil)
      assert.is_false(ok)
      assert.equal(
        "built-in pack for WARRIOR rejected (data must be a function, got nil)",
        logged[1])
    end)
  ...
  describe("BuiltinPack(class)", function()
    ...
    it("returns nil and logs the class and the wrong type when the thunk returns a non-table", function()
      Packs.RegisterBuiltinPack("WARRIOR", function() return "not a pack" end)
      local pack = Packs.BuiltinPack("WARRIOR")
      assert.is_nil(pack)
      assert.equal(1, #logged)
      assert.equal("built-in WARRIOR pack returned string, not a table", logged[1])
    end)

    it("returns nil when the thunk returns nothing at all", function()
      Packs.RegisterBuiltinPack("WARRIOR", function() end)
      assert.is_nil(Packs.BuiltinPack("WARRIOR"))
      assert.equal("built-in WARRIOR pack returned nil, not a table", logged[1])
    end)
    ...
    it("a broken class file disables only that class, not the whole registry", function()
      ...
      Packs.RegisterBuiltinPack("WARRIOR", function() return { class = "WARRIOR" } end)
      assert.is_nil(Packs.BuiltinPack("PALADIN"))
      local warrior = Packs.BuiltinPack("WARRIOR")
      assert.is_table(warrior)
