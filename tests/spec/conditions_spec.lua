  ...
  describe("describe()", function()
    ...
    it("writes each dynamic kind in words a player can read", function()
      local cases = {
        ...
        -- MG1-D5(a): `max` is the one op that ALSO passes on absence (Core/Schema.lua C.buff.make),
        -- and the wording says so rather than reading like a plain stack floor.
        { { "buff", "HOLY_POWER_BUFF", max = 3 }, "holy power buff at 3 stacks or fewer (or not up)" },
        ...
        -- MG1-D5(b): `min`/`maxRemaining` mirror `buff`'s own ops (Core/Schema.lua C.debuff.make).
